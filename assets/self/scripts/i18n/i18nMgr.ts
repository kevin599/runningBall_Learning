import { JsonAsset, loader, SpriteFrame, _decorator } from "cc";
...
export default class i18nMgr {
	...
	/**语言，默认中文 */
	private _language: Language = Language.id;
	/**文本配置 */
	...
	public setLanguage(language: Language) {
		if (this._language == language) {
			return;
		} else if (!this.language_path[language]) {
			// use default language
			console.log("Can't load this language:", language);
		} else {
			this._language = language;
			console.log("[i18nMgr] setCurLanguage:", language);
			this.loadConfig();
		}
	}
	...
	private loadConfig() {
		...
		loader.loadRes(url, JsonAsset, (err, assets) => {
			if (err == null) {
	...
	public getSpritePath(language: Language, key) {
		return "i18n/sprite/" + this.language_path[language] + "/" + key;
	}
